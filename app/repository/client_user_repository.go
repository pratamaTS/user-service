...
type ClientUserRepository interface {
	...
	ListClientUser(req *dto.APIRequest[dto.FilterRequest]) ([]dao.ClientUser, error)
...
func (u *ClientUserRepositoryImpl) ListClientUser(req *dto.APIRequest[dto.FilterRequest]) ([]dao.ClientUser, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
