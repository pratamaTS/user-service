...
func (u *MenuRepositoryImpl) SaveMenu(data *dao.Menu) (dao.Menu, error) {
	...
	if _, err := u.menuCollection.UpdateOne(ctx, filter, update, opts); err != nil {
		return dao.Menu{}, err
	}

	var out dao.Menu
	if err := u.menuCollection.FindOne(ctx, filter).Decode(&out); err != nil {
		return dao.Menu{}, err
	}
	return out, nil
}

func (u *MenuRepositoryImpl) DetailMenu(uuid string) (dao.Menu, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	var result dao.Menu
	err := u.menuCollection.FindOne(ctx, bson.M{"uuid": uuid}).Decode(&result)
	return result, err
}
...
func (u *MenuRepositoryImpl) ListMenu(req *dto.FilterRequest) ([]dao.Menu, error) {
	...
	if req.Search != "" {
		filter["$or"] = []bson.M{
			...
			{"href": bson.M{"$regex": req.Search, "$options": "i"}},
	...
	cur, err := u.menuCollection.Find(ctx, filter, opts)
	if err != nil {
		return nil, err
	}
	defer cur.Close(ctx)
...
func (u *MenuRepositoryImpl) DeleteMenu(uuid string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	_, err := u.menuCollection.DeleteOne(ctx, bson.M{"uuid": uuid})
	return err
}
